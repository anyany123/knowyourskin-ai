...
function BotCard({ title, desc, route }) {
  return (
    <Card className="rounded-2xl shadow-xl hover:shadow-2xl border-pink-300 border">
      <CardContent className="p-4 flex flex-col gap-3">
        <h2 className="text-xl font-semibold text-pink-600">{title}</h2>
        <p className="text-gray-600 text-sm">{desc}</p>
        <a href={route} className="mt-auto">
          <Button className="w-full bg-pink-600 text-white hover:bg-pink-700">
            Start
          </Button>
        </a>
        <p className="text-xs text-right text-gray-400 mt-2">
          Bot created by Priyanka Patel
        </p>
      </CardContent>
    </Card>
  );
